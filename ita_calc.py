...
def calculate_ita(file_path, file_path_crite, file_paht_form):
    ...
    for col in colunas_id:
        if col not in df.columns:
            df[col] = np.nan
    blocos = [
        (["TEMPO UFPR - SEM", "ch-integralizada", "ch_media_esperada"], "risco_ch_integralizada", "peso_ch_integralizada",
         "nota_parcial_ch_integralizada"),
        (["CH ABAIXO", "CH MTO ABAIXO","IRA SEM","porcentagem-aprovacao","qtd-matriculada","qtd-reprovacao-por-nota"], 
        "risco_aprovacao", "peso_aprovacao",
         "nota_parcial_aprovacao"),
        (["qtd-matricula-cancelada","PORT 5 - CAN",
          "qtd-rep-frequencia","PORT 5 - FREQ"], "risco_rep_freq", "peso_rep_freq", "nota_parcial_rep_freq"),
        (["% Rep Freq 2024-2","% Rep Freq 2024-1","% Rep Freq 2023 -2","porcentagem-historica-de-reprovacao-frequencia"], 

...
| R33 | Bootstrap automático do schema/ciclos | Implementado 0.4.2 | src/services/bootstrap_service.py + app.py | test_bootstrap_v042.py | Sem dados sintéticos |
| R34 | Gestão de ciclos pela interface | Implementado 0.4.2 | pages/23_administracao.py | test_cycle_management_v042.py | Criar/ativar/encerrar/reabrir |
| R35 | Produção Streamlit sem scripts manuais | Implementado 0.4.2 | app.py | teste bootstrap | init/load_sample não obrigatórios |
| R36 | Bloqueio de ciclo encerrado | Implementado 0.4.2 | pages/02_dados.py | revisão/teste serviço | Evita novas operações |
| R37 | Persistência externa crítica em produção Streamlit | Implementado 0.4.2 | readiness_service.py | test_readiness_v042.py | PostgreSQL recomendado |

...
def production_readiness(session) -> list[dict]:
    checks=[]
    def add(name, ok, level, detail): checks.append({"check":name,"ok":bool(ok),"level":level,"detail":detail})
    prod=is_production()
    ...
    add("Banco disponível", database_ping(), "CRITICO", f"Backend: {database_backend()}")
    add("Persistência recomendada", database_backend()!="sqlite", "ALTO", "No Streamlit com múltiplos usuários, PostgreSQL externo persistente é recomendado; SQLite é aceito apenas se a infraestrutura garantir persistência e baixa concorrência.")
    ...
    add("Template de secrets", secret_example, "MEDIO", "Use secrets reais somente no ambiente de deploy; não versione secrets.toml.")
